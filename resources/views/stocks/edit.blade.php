            ...
            <form action="{{ route('stocks.update', $get->id) }}" method="post">
                ...
                <div class="row">
                    <div class="col-md-3">
                        ...
                        <input type="text" name="nopo" class="form-control {{ $errors->has('nopo') ? 'is-invalid' : ''}}" value="{{ $get->nopo }}" required>
                        {!! $errors->first('nopo', '<span class="invalid-feedback">:message</span>') !!}
                    </div>
                    <div class="col-md-3">
                        <label for="">Tanggal PO</label>
                        <input type="date" name="tanggalpo" class="form-control {{ $errors->has('tanggalpo') ? 'is-invalid' : ''}}" value="{{ $get->tanggalpo }}" required>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" name="supplier" class="form-control {{ $errors->has('supplier') ? 'is-invalid' : ''}}" value="{{ $get->supplier }}" required>
